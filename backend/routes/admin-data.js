...
const modelsConfig = {
  ...
  dispatch: {
    baseSelect: "SELECT d.id, COALESCE(o.order_number, ab.booking_number) AS reference_number, IF(o.id IS NOT NULL, 'Online Order', 'Advance Booking') AS reference_type, d.dispatch_type, d.status, DATE_FORMAT(d.dispatch_date, '%Y-%m-%d') AS dispatch_date, d.bus_number, d.driver_name, d.driver_mobile, d.courier_company, d.docket_number FROM dispatches d LEFT JOIN orders o ON o.id = d.order_id LEFT JOIN advance_bookings ab ON ab.id = d.advance_booking_id",
    baseWhere: "WHERE 1=1",
